...
async function getFoods(lat, long) {
    if (!(lat && long)) {
        const response = await fetch(`http://mysmallmeal.shop/api/shop?lat=${latitude}&lng=${longitude}`);
        return await response.json()
    } else {
        const response = await fetch(`http://mysmallmeal.shop/api/shop?lat=${lat}&lng=${long}`);
        return await response.json()
    }
}
...
async function NoGeoDontWorry() {
    const response = await fetch(`http://mysmallmeal.shop/api/shop?lat=${latitude.toFixed(7)}&lng=${longitude.toFixed(7)}`);
...
function sendLike(user, headers, body) {
    ...
    fetch(`http://mysmallmeal.shop/api/like`, init)
...
function showBookmarks(user) {
    ...
    fetch(`http://mysmallmeal.shop/api/like?uuid=${user}`)
        .then((r) => r.headers.get('content-type').includes('json') ? r.json() : r.text())
        .then((res) => {
...
// 즐겨찾기 클릭시 모달창 오픈
function popUp(ssid) {
    fetch(`http://mysmallmeal.shop/api/detail?ssid=${ssid}`).then((restaurant) => {
...
window.addEventListener('hashchange', async () => {
    ...
    const response = await fetch(`http://mysmallmeal.shop/api/shop?order=${hash}&lat=${latitude}&lng=${longitude}`);
    let restaurants = await response.json()
    emptyCards()
    restaurants.forEach((restaurant, index) => {
        let i = index % 3
...
function search() {
    ...
    console.log(init)
    fetch(`http://mysmallmeal.shop/api/address`, init)
        .then((r) => r.headers.get('content-type').includes('json') ? r.json() : r.text())
        .then((result) => {
